...
<div class="space-y-6">
    ...
    @if(session('success'))
        <div class="bg-green-500/20 border border-green-500/30 text-green-200 px-4 py-3 rounded-lg flex items-center">
    ...
    @if(session('warning'))
        <div class="bg-yellow-500/20 border border-yellow-500/30 text-yellow-200 px-4 py-3 rounded-lg flex items-center">
            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="bg-slate-900 rounded-3xl shadow-xl overflow-hidden border border-slate-700">
        <!-- Cover Photo Section -->
        <div class="relative h-64 bg-gradient-to-r from-blue-600 via-cyan-500 to-teal-500">
            @if($user->cover_photo)
            ...
            <div class="absolute top-4 right-4">
                <button onclick="document.getElementById('coverPhotoInput').click()" 
                        class="flex items-center px-4 py-2 bg-slate-800/90 backdrop-blur-sm text-slate-200 font-medium rounded-lg hover:bg-slate-700/90 transition shadow-lg border border-slate-600">
            ...
            <div class="flex flex-col md:flex-row md:items-end md:justify-between">
                ...
                <div class="flex flex-col md:flex-row md:items-end -mt-16 md:-mt-20">
                    ...
                    <div class="relative group">
                        <div class="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-slate-900 shadow-xl overflow-hidden bg-gradient-to-br from-blue-500 to-cyan-500">
                    ...
                    <div class="mt-4 md:mt-0 md:ml-6 md:mb-4">
                        ...
                        <p class="text-lg text-cyan-400">@<span>{{ $user->username }}</span></p>
                ...
                <div class="mt-4 md:mt-0 md:mb-4">
                    <button onclick="window.scrollTo({top: document.getElementById('settingsSection').offsetTop - 80, behavior: 'smooth'})" 
                            class="w-full md:w-auto px-6 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-cyan-700 transition shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/50">
    ...
    <!-- Tabs Navigation -->
    <div id="settingsSection" class="bg-slate-900 rounded-3xl shadow-xl border border-slate-700" x-data="{ activeTab: 'account' }">
        <div class="border-b border-slate-700">
            <nav class="flex -mb-px">
                <button @click="activeTab = 'account'" 
                        :class="activeTab === 'account' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200 hover:border-slate-600'"
                ...
                <button @click="activeTab = 'password'" 
                        :class="activeTab === 'password' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200 hover:border-slate-600'"
                        class="w-1/2 py-4 px-1 text-center border-b-2 font-medium text-sm transition">
                    <div class="flex items-center justify-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div x-show="activeTab === 'account'" x-cloak class="p-6">
            <form action="{{ route('profile.update') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required
                               class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 @error('name') border-red-500 @enderror">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username" value="{{ old('username', $user->username) }}" required
                               class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 @error('username') border-red-500 @enderror">
                        @error('username')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required
                               class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 @error('email') border-red-500 @enderror">
                        @error('email')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                        @if(!$user->email_verified_at)
                            <p class="mt-1 text-sm text-yellow-400">Email not verified</p>
                        @endif
                    </div>

                    <!-- School -->
                    <div>
                        <label for="school" class="block text-sm font-medium text-slate-300 mb-2">School/Institution</label>
                        <input type="text" name="school" id="school" value="{{ old('school', $user->school) }}"
                               placeholder="Southern de Oro Philippines College"
                               class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 placeholder-slate-400 @error('school') border-red-500 @enderror">
                        @error('school')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Required Hours -->
                    <div>
                        <label for="required_hours" class="block text-sm font-medium text-slate-300 mb-2">Required OJT Hours</label>
                        <input type="number" name="required_hours" id="required_hours" value="{{ old('required_hours', $user->required_hours ?? 590) }}"
                               min="1" max="2000" step="1"
                               class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 @error('required_hours') border-red-500 @enderror"
                               onkeyup="calculateRequiredDays()">
                        @error('required_hours')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-sm text-slate-400">
                            Equivalent to <span id="required_days" class="font-semibold text-blue-400">{{ $user->getRequiredDays() }}</span> days
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    <h4 class="text-sm font-semibold text-slate-300 mb-3">Your OJT Progress</h4>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <p class="text-xs text-slate-400">Total Rendered</p>
                            <p class="text-lg font-bold text-green-400">{{ number_format($user->getTotalRenderedHours(), 2) }} hrs</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-400">Remaining</p>
                            <p class="text-lg font-bold text-orange-400">{{ number_format($user->getRemainingHours(), 2) }} hrs</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-400">Progress</p>
                            <p class="text-lg font-bold text-blue-400">{{ number_format($user->getProgressPercentage(), 2) }}%</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6 bg-gradient-to-br from-yellow-500/20 to-orange-500/20 border-2 border-yellow-500/30 rounded-xl p-6">
                    <div class="flex items-start space-x-3 mb-4">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-bold text-yellow-200 mb-1">⚠️ Face Recognition Required</h3>
                            ...
                            <div id="face_registration_status" class="hidden mb-3">
                                <div class="flex items-center space-x-2 text-green-200 bg-green-500/20 px-4 py-2 rounded-lg border border-green-500/30">
                            ...
                            <button type="button" onclick="openProfileFaceCapture()" 
                                    class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-semibold rounded-lg hover:from-yellow-600 hover:to-orange-600 transition shadow-lg shadow-yellow-500/30 hover:shadow-xl hover:shadow-yellow-500/50">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/>
                                </svg>
                ...
                <div class="flex items-center justify-end space-x-4 pt-4 border-t border-slate-700">
                    <button type="button" onclick="window.location.reload()" 
                            class="px-6 py-2.5 border border-slate-600 text-slate-300 font-medium rounded-lg hover:bg-slate-800 transition">
                        Cancel
                    </button>
                    <button type="submit" 
                            class="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-cyan-700 transition shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/50">
                        Save Changes
                    </button>
                </div>
            </form>
            ...
            <form action="{{ route('profile.password') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="password" name="current_password" id="current_password" required
                           class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 @error('current_password') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="password" name="password" id="password" required
                           class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200 @error('password') border-red-500 @enderror">
                    @error('password')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="password" name="password_confirmation" id="password_confirmation" required
                           class="w-full px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-200">
                </div>
...
<div id="faceRegistrationModal" class="hidden fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center p-4">
    <div class="bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-700">
        <!-- Modal Header -->
        <div class="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-6 py-4 rounded-t-2xl flex items-center justify-between">
            ...
            <button onclick="closeFaceRegistration()" class="text-white hover:text-slate-200">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4">
                <h4 class="font-semibold text-blue-200 mb-2">📋 Instructions:</h4>
            ...
            <!-- Camera Container -->
            <div class="relative">
                <video id="faceVideo" class="w-full rounded-lg border-4 border-slate-600" autoplay muted playsinline style="max-height: 400px;"></video>
            ...
            <!-- Status Messages -->
            <div id="faceStatus" class="text-center text-sm text-slate-300"></div>

            <!-- Action Buttons -->
            <div class="flex items-center justify-end space-x-4">
                <button type="button" onclick="closeFaceRegistration()" 
                        class="px-6 py-2.5 border border-slate-600 text-slate-300 font-medium rounded-lg hover:bg-slate-700 transition">
                    Cancel
                </button>
                <button type="button" id="captureFaceBtn" onclick="captureFace()" disabled
                        class="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-cyan-700 transition shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed">
                    Capture & Register
                </button>
            </div>
        </div>
